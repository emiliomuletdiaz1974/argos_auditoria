...
| ARG-081-082 | `harden.sh` desactiva `usb-storage` y añade a `/etc/fstab` con `cat >>`; la clave de recuperación en `/root/recovery-key.txt`; un resellado sin quien apunte el slot viejo; la puntuación tomada del escáner | `usb-storage` disponible como excepción CIS 1.1.1.10; ficheros escritos solo si cambian; la clave solo en la consola; resellado en dos pasos (`prepare` y `confirm`); puntuación calculada por la puerta | La esclusa de ARG-090 necesita soportes extraíbles; `cat >>` no es idempotente; una clave en un fichero sobrevive en el disco que protege | La prueba en contenedor ejecuta `harden.sh` dos veces sin cambios y busca la clave de recuperación en todo el sistema de ficheros sin encontrarla | **Propuesta**, pendiente de aprobación |
| ARG-086 | cosign y `skopeo`; un contenedor del actualizador en el compose (ADR-0014) | El actualizador de desarrollo corre en el anfitrión y conduce Docker Compose; la API verifica y encola | Compose resuelve las rutas de los volúmenes en el cliente: desde un contenedor en Docker Desktop no existen; y el actualizador necesitaría el socket de Docker (administrador del anfitrión) | Comprobamos que `docker build` y los volúmenes relativos dependen del cliente; la prueba de integración aplica y deshace contra el compose real | **Propuesta**, pendiente de aprobación |
| ARG-089 | Volcado diario más WAL para punto en el tiempo, instantánea raft de Vault, `backup.sh` y prueba sobre `postgres:16` | Volcado diario sin WAL hasta la Fase 10; en desarrollo, exportación de los secretos de `argos/`; scripts en Python; prueba sobre la imagen del entorno sin red; temporizadores de systemd | El RPO del contrato (24 h) lo cumple el volcado diario; el Vault de desarrollo no tiene raft; `postgres:16` no tiene AGE ni pgvector | Restauramos a mano un volcado real en la imagen del entorno y el grafo respondió igual (67 nodos en los dos); la prueba de integración restaura, verifica y falla con la copia corrompida | **Propuesta**, pendiente de aprobación |
