...
- `F09-99` — **la prueba de restauración compara la copia con los recuentos de su momento, no con la producción de ahora** — `make restore-test` sobre la copia de desarrollo de las 06:13 falló. Dio por ausente `argos.closed_sessions`, creada después por la migración 0039, y por vacías tablas que entonces lo estaban. La copia estaba bien. El día de una actualización con migraciones, la prueba mensual habría dado un falso fallo crítico. Ahora el backup guarda `counts.json` junto al volcado y la prueba compara con él; una copia antigua sin ese fichero se sigue comparando con producción. Un test puro lo fija, y una copia nueva pasa.
- `F09-99` — **la prueba de aceptación reutiliza las piezas de cada tarea contra el entorno real** — la batería y la comprobación del dossier se ejecutan tal cual. El backup, la restauración, las actualizaciones (también por la esclusa) y la postura se encadenan en la prueba con las funciones de producción, no con dobles.
- `F09-16` — **el dossier recoge todas las medidas y todos los controles, y el organismo decide la aplicabilidad** — las fuentes del proyecto solo nombran las familias del ENS. Escribimos las 73 medidas del Anexo II del RD 311/2022 y los 93 controles del Anexo A de la ISO/IEC 27001:2022 con sus identificadores oficiales, y un test comprueba que están todos y solo una vez. No marcamos cuáles aplican a categoría media: eso lo decide el organismo en su Declaración de Aplicabilidad, y así queda dicho en el dossier.
- `F09-16` — **«implementada» frente a «implementada en desarrollo»** — la tarea pedía no presentar como implementado en el appliance lo que solo existe en el compose. «Implementada» es lo que está en el código o en el proceso de desarrollo y vale igual en cualquier despliegue (la matriz de autorización, el SBOM, el diario). «Implementada en desarrollo» es lo que depende de la configuración del despliegue y solo se ha probado en el compose (Keycloak, mTLS, Prometheus). El test rechaza una medida «implementada» con evidencia en `deploy/dev/`.
- `F09-16` — **lo organizativo, físico y de personal es «responsabilidad del organismo»** — el producto no puede cumplirlo por sí mismo. Cuando aporta algo (alertas, diagnóstico, cierre de sesión) se dice en la nota, sin cambiar el estado.
- `F09-16` — **los SBOM no se versionan; el paquete los copia de `dist/sbom/`** — cambian con cada release y ocupan megas. El manifiesto de release firmado ya lleva su huella, y el paquete del dossier los incluye con la suya.
- `F09-16` — **el PDF del dossier va apaisado y con letra más pequeña** — revisamos el PDF de las medidas ENS pasándolo a imagen, y la última columna (tarea y nota) se salía de la página. En A4 apaisado, con 8 puntos en las tablas, todo cabe; lo comprobamos igual, y un test fija la orientación.
- `F09-16` — **orden de los tests** — el del dossier falló antes de existir los documentos y, como pasó a la primera al escribirlos, comprobamos a mano que el verificador detecta cada tipo de error (ruta inexistente, evidencia del compose, pendiente sin tarea, estado desconocido, implementada sin evidencia). Los del paquete fallaron antes de `build_security_pack`. El de la orientación se escribió después de corregir el PDF.
- `F09-32` — **se cierra la sesión (`sid`), no el token** — el cierre de sesión solo tiene el refresco, no el token de acceso, pero los dos llevan el `sid` de la misma sesión del realm. Apuntar la sesión rechaza cualquier token de acceso que haya emitido, incluido uno que otra pestaña refrescara antes del cierre.
- `F09-32` — **la sesión se apunta solo si el realm aceptó revocar ese refresco** — el `sid` se lee del refresco sin verificar su firma. Si la API lo apuntara siempre, cualquiera con el `sid` de otra persona (va en sus tokens) cerraría su sesión con una cookie inventada. La revocación en Keycloak verifica la firma, así que solo cierra quien tiene el refresco auténtico. Un test lo comprueba con un realm que rechaza la cookie.
- `F09-32` — **una tabla compartida con caché de 5 segundos, no memoria ni NATS KV** — la tarea pedía decidirlo. La memoria de un proceso no vale con varias réplicas. NATS KV añadiría a la API una dependencia que hoy no tiene en la ruta de cada petición. La base ya está en esa ruta y la tabla se limpia sola al escribir. El precio es que un token puede seguir valiendo hasta 5 segundos tras el cierre en otra réplica.
- `F09-32` — **15 minutos de cierre** — más que la vida de un token de acceso del realm (5 minutos), con margen por si se alarga. Pasado ese tiempo no hay token de esa sesión que valga, y la fila se borra.
- `F09-32` — **la prueba de la batería usa una sesión real de la consola** — la primera versión cerraba un refresco del cliente `argos-tests`, y Keycloak rechazaba en silencio revocarlo con el cliente de la consola: el cierre no cerraba nada. Ahora la batería abre la sesión como la consola (código con PKCE, canje en `/api/v1/auth/session` y cookie de refresco) y la cierra con `/api/v1/auth/logout`. La vimos fallar contra la API sin el arreglo (`200`) y pasar tras reconstruirla (`401`).
- `F09-32` — **el cierre desde el propio Keycloak (back-channel logout) queda fuera** — la tarea lo daba como opcional. La consola cierra siempre por la API, y la sesión cerrada en el realm deja de refrescarse de todos modos. Queda en pendientes.
- `F09-32` — **orden de los tests** — los puros fallaron por importación antes de existir `argos_api.sessions`, y la prueba de la batería falló contra la API desplegada sin el arreglo. La migración y la prueba del almacén contra PostgreSQL se escribieron a la vez y pasaron a la primera.
- `F09-15` — **los tokens de la batería se piden con `Host: keycloak:8080`** — la API desplegada valida el emisor que ve dentro de la red del compose, y Keycloak construye el emisor a partir de la cabecera `Host`. Comprobamos que un token pedido así lleva `iss=http://keycloak:8080/realms/argos` y que la API desplegada lo acepta. Así la batería ataca el contenedor como lo haría otro servicio de la red.
- `F09-15` — **el realm de desarrollo gana `admin.test` (con TOTP), `auditor.test` y dos clientes de prueba** — para probar la matriz con los cuatro roles hacía falta un usuario de cada uno. `argos-other` da tokens sin la audiencia de la API y `argos-expiring` tokens de 5 s, para probar el rechazo de otra audiencia y de un token caducado con tokens reales, no fabricados. Todo lleva la marca de solo desarrollo.
- `F09-15` — **la matriz solo ataca lo que cada rol no puede hacer** — lo permitido cambiaría el estado del entorno. Después de las denegaciones comprobamos que ninguna dejó un asiento del diario con el actor de ese usuario: una denegación que deja efecto no es una denegación.
- `F09-15` — **los roles que deciden no tienen token sin su código** — no se puede probar un token de `platform_admin` o `dpo_reviewer` sin segundo factor contra la API, porque Keycloak no lo emite. La batería lo comprueba en el realm (`401`) y limpia el bloqueo por fuerza bruta que dejan esos intentos.
- `F09-15` — **los workers no tienen identidad en Keycloak** — la tarea pedía usar en la API el token de la cuenta de servicio de un worker, y no existe: usan su rol de base de datos y mTLS. Lo más parecido es una cuenta del realm sin roles de ARGOS, y la batería comprueba que recibe `403` en las rutas.
- `F09-15` — **la batería crea su propia campaña y su propio hallazgo** — la primera versión tomaba un hallazgo del entorno y se saltaba la prueba porque no había ninguno. Ahora crea un veredicto no conforme de su campaña con las funciones del motor, así que no depende de lo que otra prueba dejara.
- `F09-15` — **SEC-058 (`Origin` ajeno) y SEC-059 (`/openapi.json` del comprobador) se corrigen aquí** — los dos eran pequeños. El middleware de mismo origen rechaza las mutaciones con `Origin` de otro origen o `null`, y deja pasar las peticiones sin `Origin` (clientes sin navegador). Sus tests se escribieron antes que el middleware. El test del comprobador se escribió después de quitar `openapi_url`, y lo vimos pasar contra el contenedor reconstruido.
- `F09-15` — **SEC-060 (token válido tras cerrar sesión) pasa a la tarea F09-32** — con varias réplicas de la API el conjunto de sesiones cerradas tiene que ser compartido, y eso no es un arreglo pequeño. La prueba queda en la batería como `xfail` estricto: cuando F09-32 lo corrija, el test fallará por pasar y habrá que quitar la marca.
- `F09-14` — **`usb-storage` no se desactiva y queda como excepción CIS documentada** — el `harden.sh` del documento de fase lo desactivaba, pero la esclusa (ARG-090) del mismo documento solo trabaja con soportes extraíbles. Nota de desviación ARG-081-082, propuesta.
- `F09-14` — **la idempotencia se prueba por contenido, permisos y fecha de cada fichero de `/etc`** — la primera versión de `ensure_line` hacía `touch` de `/etc/fstab` y cambiaba su fecha en cada ejecución. El test lo detectó y ahora el fichero solo se crea si no existe.
- `F09-14` — **lo que necesita núcleo o repositorio es un error, salvo con `ARGOS_HARDEN_ALLOW_SKIP=1`** — en un contenedor no se puede cargar sysctl ni las reglas de auditoría, ni instalar auditd sin red. La prueba lo salta con una línea `SKIP` que dice por qué y comprueba que, sin ese permiso, la misma ejecución falla.
- `F09-14` — **la clave de recuperación no pasa por ningún fichero** — la tarea lo pedía y el documento de fase la escribía en `/root`. Se captura en una variable, se imprime y se borra. La prueba la busca en todo el sistema de ficheros del contenedor, salvo en el stub que la genera.
- `F09-14` — **el resellado se parte en `prepare` y `confirm`** — el documento retiraba el slot de `pending-old-tpm-slot` sin que nadie lo escribiera. `prepare` lo escribe antes de reiniciar, y `confirm` sella primero y retira después. La prueba comprueba ese orden.
- `F09-14` — **la puerta calcula la puntuación en lugar de leerla del informe** — así decide igual con cualquier escáner que produzca las comprobaciones con su resultado. Las excepciones dejan pasar fallos conocidos, pero no suben la puntuación. Los números CIS de las excepciones se contrastarán con la versión del benchmark del escáner en F09-90.
- `F09-14` — **los tests se escribieron antes que los scripts** — fallaron por no existir `tools/cis_gate.py` ni los scripts. Después falló el de idempotencia (el `touch`), y fallaron dos del propio test (la codificación de la salida de Docker y el stub que contiene la clave) antes de corregirlos.
- `F09-13` — **la esclusa corre en la API y deja lo que entra donde su servicio lo recoge** — la tarea pide las rutas en la API y la API ya tiene la evidencia, la cola del actualizador y el almacén de diagnóstico. Una actualización verificada queda en la bandeja y la cola del actualizador (`via: airgap`), como la ruta de F09-10. Las respuestas de sello y el contenido se aplican en la API, porque su verificación ya vive en funciones de dominio (`accept_reply`, `load_bundle`).
- `F09-13` — **`svc_api` puede insertar las cuádruplas de contenido (migración 0038)** — `load_bundle` verifica la huella fijada, la firma y el retroceso, y solo después guarda la versión con sus cuádruplas. La API ya podía insertar la fila del bundle, no sus cuádruplas. Le damos `INSERT` y nada más: el almacén versionado no admite actualizar ni borrar. Descartamos un contenedor aparte para la esclusa: repetiría las credenciales, la evidencia y las colas que la API ya tiene.
- `F09-13` — **los `.tsq` y `.tsr` se nombran con 32 hexadecimales del SHA-256 de la clave del objeto** — la clave (`campaigns/<id>/root.sig`) no cabe en un nombre de fichero. El nombre solo sirve para encontrar el objeto en cola, y `accept_reply` verifica la respuesta contra el objeto y su nonce, así que un nombre cambiado solo lleva a un rechazo.
- `F09-13` — **un enlace, una carpeta o un dispositivo se rechazan con `lstat`, antes de abrirlos** — y la comprobación va antes que buscar el importador. El test del enlace simbólico se salta si el sistema no deja crearlos; en la máquina de trabajo sí se ejecutó.
- `F09-13` — **el paquete de actualización se desempaqueta con el filtro `data` de `tarfile`** — Python 3.12 rechaza con él rutas absolutas, `..`, enlaces hacia fuera y dispositivos. Un test lo comprueba con un miembro `../../escaped.txt`, que no llega a escribirse.
- `F09-13` — **el test de la barrera de IA de la API comparaba un prefijo** — `startswith("argos_ai")` daba positivo con `argos_airgap`. Ahora compara el paquete exacto o sus módulos, y un test comprueba que `argos_airgap` no cuenta y `argos_ai.guardrails` sí.
- `F09-13` — **una exportación rechazada también va al diario** — la integración lo mostró: el intento de exportar un tipo fuera de la lista queda como `refused` en el diario y en el registro de seguridad, igual que una importación rechazada.
- `F09-13` — **los tests se escribieron antes que el código** — los puros y los de la API fallaron por importación antes de existir el paquete y las rutas; el de la vista, antes de existir la vista. El de una carpeta con nombre de actualización falló por el orden de las comprobaciones antes de corregirlo.
- `F09-12` — **el volcado lógico basta para el grafo de AGE** — antes de diseñar nada restauramos a mano un `pg_dump` real de la base de desarrollo en un PostgreSQL desechable. El `graphid` de `ag_graph` no coincide con el OID del esquema nuevo (17383 frente a 17558), pero AGE 1.5 usa su propio identificador y la consulta cypher devolvió los mismos 67 nodos que producción. La prueba de restauración compara ese recuento siempre.
- `F09-12` — **la prueba de restauración lee la copia con `docker exec`, sin red ni puertos** — la tarea prohíbe publicar puertos y pide que la prueba no pueda llegar a producción por error. Con `--network none` no hay ruta. Los verificadores del diario y del registro de seguridad trabajan sobre las filas exportadas en JSON, con la misma función `verify_entries` del diario v1.
- `F09-12` — **una copia más vieja que producción no es un fallo; una tabla vacía o ausente, sí** — producción sigue escribiendo mientras se prueba, y algunas tablas se vacían a propósito (claves de idempotencia, colas). Además comprobamos que el último asiento del diario de la copia es el mismo en producción: la copia es un prefijo de la historia real.
- `F09-12` — **los scripts son de Python y no de bash** — corren igual en el anfitrión de desarrollo (Windows) y en el nodo, se prueban con dobles (limpieza con excepción, contraseña fuera de la línea de órdenes) y reutilizan el verificador del diario y el del registro de seguridad. Nota de desviación ARG-089.
- `F09-12` — **la cadencia es de systemd en el nodo, no de Temporal ni de `ofelia`** — la tarea pedía decidirlo. Los workers de Temporal y un contenedor de `ofelia` necesitarían el socket de Docker o `kubectl`, en contra de la postura de F09-03. El backup corre donde está el orquestador, como el actualizador y el recolector de diagnóstico.
- `F09-12` — **el PostgreSQL de backup es un usuario efímero de Vault del rol `svc_backup`** — con `pg_read_all_data` y sin escritura, salvo en su tabla de resultados, su prefijo del diario (`backup.%` como `system:backup`) y el registro de seguridad. Se revoca al terminar cada ejecución. El volcado se hace dentro del contenedor de PostgreSQL, con la contraseña por la entrada estándar.
- `F09-12` — **la contraseña de restic de desarrollo es un valor fijo `dev-only-`** — el Vault de desarrollo pierde su estado al reiniciarse, y una contraseña nueva cada vez dejaría ilegibles las copias anteriores. Sigue la convención de los demás secretos de desarrollo.
- `F09-12` — **las alertas se prueban con `promtool test rules`** — un fallo fija `argos_backup_last_restore_test_success` a 0 (crítica), y sin ninguna prueba que haya pasado el instante es 0, así que la alerta de prueba caducada salta sola. Con muestras diarias la serie caducaba a los 5 minutos y ninguna alerta saltaba: las series de la prueba van por minuto.
- `F09-12` — **en la integración, la copia corrompida va antes que la buena** — las dos escriben en la tabla de desarrollo, que es de solo inserción. Así el entorno termina con la última prueba en verde y no con la alerta crítica encendida.
- `F09-12` — **los tests se escribieron antes que el código** — los puros fallaron por importación antes de existir los módulos; la integración falló por la métrica y las reglas antes de escribirlas.
- `F09-12` — **`make secrets` se repite después del commit** — gitleaks revisa los commits, no el árbol de trabajo. La verificación de F09-11 corrió antes de su commit y tres falsos positivos del commit `34fc10a` (la cita de la anotación de tipo de la clave de firma de los tests del actualizador y el token sintético que el test de diagnóstico planta a propósito) no salieron hasta esta tarea. Añadimos sus huellas a `.gitleaksignore` con el motivo, y desde ahora volvemos a pasar `make secrets` sobre el commit antes de fusionar.
- `F09-12` — **el backup habla con PostgreSQL con TLS verificado, dentro y fuera del contenedor** — la primera versión usaba `sslmode=require` y la suite completa la rechazó (`test_tls_verification.py`, F09-06): cifra sin comprobar a quién. Comprobamos que el certificado del servidor cubre `127.0.0.1` y `localhost`. El volcado usa la CA que el contenedor ya tiene (`/run/tls/ca.crt`) y el anfitrión la de `ARGOS_TLS_DIR`, ambos con `verify-full`.
- `F09-11` — **el depurador de ARG-060 pasa al SDK de conectores** — la tarea pedía `argos_ai.guardrails.scrub_input`, pero la API construye el paquete y ADR-0012 le prohíbe importar la capa de IA (lo comprueba `test_no_ai_imports_pure.py`). Movimos el bucle a `argos_connector.validators.scrub_identifiers`, junto a los validadores que ya usaba, y `scrub_input` delega en él. Es la misma tabla y la misma función, sin duplicar, y los tests del gateway siguen pasando sin cambios.
- `F09-11` — **la recogida corre junto al orquestador y la API solo encola, muestra y cifra** — el contenedor de la API no tiene el socket de Docker ni debe tenerlo (F09-03), y en el appliance el recolector va en el nodo con `kubectl`, como dice el documento de fase. En desarrollo corre en el anfitrión, como el actualizador (nota ARG-086). Por eso `POST /support/diagnostics` responde `202` con un identificador y la vista previa se lee con `GET`, en lugar de devolverla en la misma llamada como decía la tarea.
- `F09-11` — **los inspectores son un puerto propio de solo lectura, no el `Orchestrator` del actualizador** — el del actualizador despliega. Reutilizarlo daría a quien recoge un diagnóstico la capacidad de cambiar servicios.
- `F09-11` — **cifrar exige la huella del índice aprobado y que cada fichero siga coincidiendo** — la API cifra los bytes que mostró, no una recogida nueva. Lo probamos con otro índice y con un fichero editado en disco después de la vista previa: en los dos casos no hay paquete (`422`).
- `F09-11` — **cifrar el paquete pide segundo factor** — la tarea solo pedía `platform_admin`. Lo que sale del appliance es de la misma familia que `system.update`, y `platform_admin` ya tiene TOTP. Pedir y leer la vista previa no lo exige.
- `F09-11` — **el depurador del paquete añade secretos, credenciales en URL, JWT, `Bearer`, pares `password=…` y correos a los identificadores de ARG-060** — la tarea pide que un correo salga depurado, y los identificadores de los guardarraíles no incluyen correos. Los valores de los secretos que el recolector ve en el entorno se sustituyen enteros donde aparezcan. No los añadimos a la tabla del gateway: cambiaría lo que ve el modelo sin una tarea de la Fase 06.
- `F09-11` — **los eventos son los del ciclo de vida, no los `exec_*`** — con el compose real, todos los eventos de la ventana eran `exec_create/start/die` de los healthchecks: ruido, y además cada uno llevaba la orden completa. Filtramos por creación, arranque, reinicio, parada, muerte, OOM, destrucción y cambios de salud.
- `F09-11` — **las fuentes simuladas (`source-*`) quedan fuera del paquete de desarrollo** — la prueba de integración encontró el recurso compartido y el usuario de `source-smb` en los nombres de sus variables (`ACCOUNT_argos_ro`, `SAMBA_VOLUME_CONFIG_clinical`), que son valores de Vault. En el appliance esas fuentes son sistemas del cliente, y el paquete nunca los inspecciona.
- `F09-11` — **un contenedor sin registros no detiene el paquete** — el contenedor huérfano `challenge-api` (retirado en F08-17) hacía fallar `docker compose logs`. Ahora su fichero dice que no hay registros y con qué código respondió el orquestador.
- `F09-11` — **la prueba de integración no cuenta `127.0.0.1` como valor de Vault** — es el host de las fuentes simuladas en la configuración de conectores, y aparece en cada registro de acceso de un healthcheck. No identifica nada. Queda escrito en el test y en las limitaciones del módulo: el recolector no lee Vault, así que un valor de configuración no secreto guardado allí puede aparecer en un registro.
- `F09-11` — **los tests se escribieron antes que el código y los vimos fallar** — primero fallaron por importación, antes de existir el paquete, las rutas y la vista. Después fallaron el del servicio sin registros, el de las fuentes simuladas y el de los eventos `exec_*`, antes de cada arreglo.
- `F09-11` — **la anotación `key: Ed25519PrivateKey = KEY` de los tests del actualizador va a `.gitleaksignore`** — gitleaks la lee como una clave genérica. Salió en esta tarea y no en F09-10 porque gitleaks revisa los commits, y la verificación de F09-10 corrió antes de su commit. Añadimos la huella con su motivo, sin tocar la regla.
