-- ARG-005 · Esquema núcleo y diario encadenado v1 (ADR-0002, Nota de Desviación ARG-005)
...
-- Sistemas del cliente registrados para verificación (los puebla C1/C2). Ids UUID v7 desde la aplicación.
CREATE TABLE argos.systems (
  ...
  owner                  text,
  connection             jsonb NOT NULL,          -- referencia a secreto, nunca la credencial
...
CREATE TABLE argos.campaigns (
  ...
  status      text NOT NULL DEFAULT 'borrador'
              CHECK (status IN ('borrador','aprobada','en_ejecucion','en_revision','cerrada')),
...
-- ── Diario de auditoría encadenado v1: solo inserción vía journal_append ──
...
LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'audit_journal es inmutable (operación %)', TG_OP;
END $$;
...
-- lp(x) = u32be(longitud en bytes) ‖ UTF-8(x)
...
-- Hash puro del asiento; debe coincidir con argos_comun.diario.calcular_hash
...
BEGIN
  IF p_actor IS NULL OR p_action IS NULL OR p_payload_canon IS NULL THEN
    RAISE EXCEPTION 'journal_append: argumentos nulos';
  END IF;
  -- Serializa a todos los escritores: sin esto dos transacciones enlazan con la misma cabeza
